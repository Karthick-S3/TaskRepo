...
CREATE OR REPLACE PACKAGE BODY getcompany AS
  ...
  BEGIN 
    ...
    SELECT 
        ...
        cd.establish_date, cd.REVENUE , cd.cid, cd.sid, cd.cityid, cu.currency , cd.currencyid, bu.budgetid,bu.description,bu.BUDGETCURRENCYID,bu.budgetactive,bu.createdate,
...
select * from companydetail where companyid=162
...
select * from budgetdetailline where  budgetid= 7407
...
select * from companydetail order by companyname
...
CREATE TABLE users (
   ...
   insert into users(username,password,status) values ('MNRHO','MNRHO','Active')

select password_hash from users where username = 'Karthick' and password_hash = 'Kart@123'

SELECT * FROM users WHERE username = 'Karthick'  AND password = 'Kart123'
...
CREATE OR REPLACE PACKAGE BODY TestPackage AS
  ...
  PROCEDURE InsertTest(
    ...
    v_id test1.id%TYPE;
  BEGIN
    INSERT INTO test1 (firstname, lastname)
  ...
  EXCEPTION
    WHEN OTHERS THEN
      ...
      p_id := NULL; -- Optional: Set p_id to NULL in case of error
  END InsertTest;
  ...
  BEGIN 
    INSERT INTO TEST2(phonenumber, id)
    VALUES (p_phonenumber, p_id);
  EXCEPTION
    WHEN OTHERS THEN
      DBMS_OUTPUT.PUT_LINE('Error: ' || SQLERRM);
  END InsertTest2;
